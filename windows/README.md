...
| **OpenCode** | The `opencode-go` key OpenCode's own sign-in writes to `~/.local/share/opencode/auth.json` (or `%APPDATA%\opencode\auth.json`) → `opencode.ai/zen/go/v1/usage`, the endpoint the OpenCode dashboard reads | Rolling / weekly / monthly Go plan windows. Read only, never refreshed — run `opencode auth login` to connect or renew it. Zen pay-as-you-go credit balance has no API, so only the Go plan windows are shown. |
